...
d3.json(`./data.json?v=${cacheBuster}`, function (error, data) {
  ...
  peopleList
    ...
    .html((d) => {
      ...
      let linksHtml = "";
      if (d.website)
        linksHtml += `<a href="${d.website}" target="_blank" onclick="event.stopPropagation()">Web</a>`;
      if (d.linkedIn)
        linksHtml += `<a href="${d.linkedIn}" target="_blank" onclick="event.stopPropagation()">LinkedIn</a>`;
      if (d.gitHub)
        linksHtml += `<a href="${d.gitHub}" target="_blank" onclick="event.stopPropagation()">GitHub</a>`;
...
function showDetailedTooltip(d, x, y) {
  let sections = "";
  if (d.graduationYear)
    sections += `<div class="tooltip-section"><strong>Graduation Year:</strong> ${d.graduationYear}</div>`;
  if (d.professionalEmail)
    sections += `<div class="tooltip-section"><strong>Email:</strong><br><a href="mailto:${d.professionalEmail}">${d.professionalEmail}</a></div>`;
  if (d.website)
    sections += `<div class="tooltip-section"><strong>Website:</strong><br><a href="${d.website}" target="_blank">${d.website}</a></div>`;
  if (d.linkedIn)
    sections += `<div class="tooltip-section"><strong>LinkedIn:</strong><br><a href="${d.linkedIn}" target="_blank">${d.linkedIn}</a></div>`;
  if (d.gitHub)
    sections += `<div class="tooltip-section"><strong>GitHub:</strong><br><a href="${d.gitHub}" target="_blank">${d.gitHub}</a></div>`;
...
searchInput.addEventListener("input", function () {
  ...
  nodeElements.each(function (d) {
    ...
    const yearScore = d.graduationYear
      ? getMatchScore(d.graduationYear, query)
      : 0;
...
const closeModalBtn = shortcutsModal.querySelector(".close-modal");
...
shortcutsBtn.addEventListener("click", () => toggleModal(true));
...
// Note: Search input handles its own Escape key to clear text
